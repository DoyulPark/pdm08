...
st.subheader(f'Data Information: shape = {df.shape}') //문자열 f 포맷
st.write(f'##### features = {list(df.columns)[:-1]}') //나머지 8개의 컬럼을 마지막꺼만 빼고 
st.write(f'##### classes = {pd.unique(classes)}')//unique=pandas numpu unique 함수
st.write('***')
...
with row0_2: 
    bar_mode = st.selectbox("Select barmode", ["relative", "group"], 0) //barmode 모드 설정
